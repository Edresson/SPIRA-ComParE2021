...
class SpiraSpTv1(nn.Module):
    ...
    def __init__(self, config):
        ...
        transformer_mlp_dim = 10
        num_layers = 2
        num_classes = 1


        self.spt = SpT(input_dim = seq_len * self.num_feature, num_max_patches = self.config.dataset['num_max_patches'], num_classes = num_classes, dim =pos_embedding_dim, depth = num_layers, heads = 16, mlp_dim = transformer_mlp_dim, dropout = transformer_dropout, emb_dropout = emb_dropout, pool=pool_type)
        self.mish = Mish()
